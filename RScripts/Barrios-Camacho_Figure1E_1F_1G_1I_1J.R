...
FinalData %>% filter(NormNucP63 > 0) %>% group_by(TimePoint) %>% 
...
FINALCellProfiler_NucP63Deg_12.png <- ggplot(FinalDatas) + 
...
ggsave(filename="ActivationPaper_Figure3E", plot=final_PMA_timecourse, device="png",
       height=10, width=16, units="in", dpi=1000)



### Dunnett Test to determine significance 

DunnettTest(x=FinalDatas$NormNucP63, g=FinalData$TimePoint)
...
FinalDatas <- FinalDatas %>% mutate(p63_category = 'resting')
FinalDatas$p63_category[FinalDatas$NormNucP63 < 0.3397729] <- 'intermediate'
FinalDatas$p63_category[FinalDatas$NormNucP63 < 0.2544129] <- 'activated'


PMA_treated_HBCs_categories <- FinalDatas %>%
...
ggsave(filename="PMA_categories_HBCs", plot=plot, device="png",
...
p <- ggplot(NormNucP63, aes(factor(TimePoint), y = NormNucP63, fill=as.factor(TimePoint))) + 
  geom_violin(scale="width", trim="FALSE") + geom_boxplot(width=.2, fill="white")

p2 <- p + theme_classic() + scale_fill_viridis(discrete=TRUE, option="D")

p3 <- p2 + xlab("Treatment time (hours) prior to recovery (TPR)") + 
  ylab(expression(paste(frac(Nuclear~P63~Integrated~Density, Nuclear~Area))))


CellProfiler_PMARecovery.png <- p3 + theme(axis.text.x = element_text(color="black", 
                                                                      size=20),
                                           axis.text.y = element_text(color="black", 
                                                                      size=23), 
                                           axis.title.x=element_text(color="black", 
...
NormNucP63 %>%  group_by(TimePoint, p63_category)  %>% count() 
...
ggsave(filename="CellProfiler_PMARecovery_final_Dec2022.png", plot=final_recovery, device="png",
       height=6, width=8, units="in", dpi=1000)


## Figure IJ ## 

pma_recovery <- read.csv("12062022_PMA_recovery_categories.csv", header=TRUE)

recovery_HBC_categories <- NormNucP63 %>%
  group_by(TimePoint, p63_category) %>%
  summarize(count = n()) %>%
  mutate(percentage = count / sum(count) * 100)

plot <- ggplot(pma_recovery, aes(x = as.factor(TimePoint), y = Resting, fill = p63_categories)) +
  geom_col(position = "fill") +
  theme_linedraw(base_line_size = 0, base_rect_size = 1) +
  scale_fill_manual(values = rev(c('black','gold2','red2'))) +
  xlab('Timepoint') + ylab('Percent') + theme(axis.title = element_text(size=15)) + theme(legend.position = 'none')  +
...
ggsave(filename="ActivationPaper_PMARecoveryCategories", plot=plot, device="png",
       height=5, width=5, units="in", dpi=1000)
